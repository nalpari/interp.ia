...
import { Issue, Project } from "../project-type"
            ...
            <div key={index} className={`border p-2 rounded cursor-pointer ${isSelected ? 'bg-gray-200' : 'hover:bg-gray-100'} h-32 relative`} onClick={() => handleDateClick(day)}>
                <div className="flex items-start gap-1">
                    ...
                    <div className="flex flex-wrap gap-0.5">
                        {isStartDate && (
                            <div className="text-xs text-white bg-blue-500 px-1 py-0.5 rounded truncate max-w-[35px]">
                                Start
                            </div>
                        )}
                        {isCreatedDate && (
                            <div className="text-xs text-white bg-green-500 px-1 py-0.5 rounded truncate max-w-[35px]">
                                Created
                            </div>
                        )}
                        {isDueDate && (
                            <div className="text-xs text-white bg-yellow-500 px-1 py-0.5 rounded truncate max-w-[35px]">
                                Due
                            </div>
                        )}
                        {isEndDate && (
                            <div className="text-xs text-white bg-red-500 px-1 py-0.5 rounded truncate max-w-[35px]">
                                End
                            </div>
                        )}
                    </div>
